...
El factor de expansión (N/n) es el peso con el que vamos a ponderar cada observación. Cada observación de la muestra representa N/ni observaciones en la población. El factor de expansión corrige el sobre-muestro hecho para ciertas poblaciones a fin de tener capacidad de inferencia para ellas. Se construye tomando el inverso de la probabilidad de selección 1/(n/N) para cada observación. Sumar los factores de expansión de cada individuo de la muestra, arroja la población. 

d. ¿Qué esperaría que suceda con el error estándar de la media de una variable cuando en vez de realizar un MAS se realiza un MAE? ¿Por qué?
Se espera que error el estándar disminuya con un MAE pues éste siempre o deja intacta o reduce la varianza, comparado al MAS. El promedio de nuestra variable en la población puede presentar diferencias para ciertas categorías de población. En un MAE, podemos obtener medias más eficientes pues la muestra mantiene esos ratios a diferencia del MAS donde podríamos tener una muestra desbalanceada y una media que, aún siendo insesgada, presente mayor error estándar.     

e. Explique en sus propias palabras que es el efecto de diseño (DEFF)
El deff es el ratio de la varianza de nuestro diseño muestral comparado con la varianza que obtendríamos con un MAS (con igual tamaño de muestra). Es decir, la varianza de nuestro diseño es igual a una varianza MAS ajustada por el deff. Si éste es mayor a 1, nuestro diseño muestral es menos eficiente (mayor variación) que un diseño MAS comparable; si deff<1 es más eficiente; y si deff=1 es igualmente eficiente.  
...
#Cada peso (N/n_i) refleja el hecho que cada observación en la muestra representa N/n_i observaciones en la población total. En nuestra muestra tendremos algunas categorías sobre-muestreadas y otras sub-muestreadas y los pesos ayudan 

#De forma mecánica, tenemos que la probabilidad de selección es n_i/N y la suma de todas las probabilidades da N/N = 1. Dado que el factor de expansión es el inverso de la probabilidad de selección 

#La suma de los factores de expansión de todos los individuos, de cada uno de los estratos, de la muestra provee una estimación del tamaño del universo de estudio, es decir a nivel poblacional. Esto se debe a que los factores de expansión o ponderadores son parámetros destinados a representar la variable medida en función de la probabilidad de selección que el diseño muestral determinó para cada unidad final de muestreo (cada individuo, hogar, etc. encuestados).
```
...
```
```{r}
#Como referencia dejamos el diseño 
mas4_dw = svydesign(id = ~1 , fpc =~fpc, weights =~w , data = mas_cenagro)
svymean(~ppquechua , mas4_dw)
svytotal(~rango_edad , mas4_dw)
```
...
```{r}
#El promedio de la variable "ppquechua" para toda la muestra es un promedio ponderado. Para ello tomamos las medias por estrato encontrados en el 4.1.2 y multiplicamos cada una por los pesos/factores de expansión encontrados en el 4.1.9. 
